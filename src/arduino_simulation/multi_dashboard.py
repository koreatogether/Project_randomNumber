...
class MultiImplementationDashboard:
    ...
    def _setup_layout(self):
        ...
        self.app.layout = html.Div([
            # 헤더
            html.Div([
                html.H1("Arduino Multi-Implementation Comparison Dashboard", 
                       className="text-center mb-4"),
                html.P("Compare multiple Arduino random number generator implementations", 
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        html.Label("Actions:"),
                        html.Br(),
                        html.Button("Run All Implementations", id="run-all-btn", 
                                  className="btn btn-primary mr-2"),
                        html.Button("Stop", id="stop-btn", 
                                  className="btn btn-danger mr-2", disabled=True),
                        html.Button("Refresh Config", id="refresh-btn", 
    ...
    def _setup_callbacks(self):
        ...
        def update_status(n_intervals, status_data):
            ...
            if status_data.get("running", False):
                if self.is_running:
                    return html.Div([
                        html.P(f"Running comparison with {status_data['iterations']:,} iterations per implementation"),
        ...
        def update_speed_chart(data):
            ...
            fig = px.bar(df, x='Implementation', y='Speed (gen/sec)',
        ...
        def update_memory_chart(data):
            ...
            fig = px.bar(df, x='Implementation', y='Memory (bytes)',
        ...
        def update_constraint_chart(data):
            ...
            df = pd.DataFrame({
                ...
                'Status': ['✅ Pass' if r['constraint_violations'] == 0 else '❌ Fail' 
                          for r in results]
            })
            
            fig = px.bar(df, x='Implementation', y='Violations',
        ...
        def update_recommendation(data):
            ...
            return html.Div([
                html.Div([
                    ...
                    html.H4(data['recommended_implementation'], className="text-primary"),
                    html.P("Based on weighted scoring of performance, memory efficiency, and reliability")
    ...
    def _create_summary_cards(self, report) -> List[html.Div]:
        ...
        cards = [
            ...
            html.Div([
                html.H4(str(report.successful_implementations)),
                html.P("Successful")
            ], className="col-md-3 text-center alert alert-success"),
            
            html.Div([
                html.H4(str(report.failed_implementations)),
                html.P("Failed")
            ], className="col-md-3 text-center alert alert-danger"),
            
            html.Div([
                html.H4("🏆"),
                html.P(report.recommended_implementation)
    ...
    def run_server(self):
        """대시보드 서버 실행"""
        print(f"Starting multi-implementation dashboard on http://localhost:{self.port}")
        self.app.run(debug=self.debug, port=self.port, host='0.0.0.0')


# ==================== 편의 함수들 ====================

def create_multi_dashboard(port: int = 8051, debug: bool = True) -> MultiImplementationDashboard:
    """다중 구현 대시보드 생성"""
