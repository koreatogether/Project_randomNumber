...
from real_arduino_sim import RealArduinoImplementationGenerator, test_real_arduino_implementations
...
class StatisticalAnalyzer:
    ...
    def _load_implementations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [impl for impl in config.get('implementations', []) if impl.get('enabled', True)]
    ...
    def analyze_all_implementations(self, iterations: int = 10000, seed: int = 12345) -> Dict[str, Any]:
        """모든 구현에 대한 상세 통계 분석"""
        print(f"=== Statistical Analysis of {len(self.implementations)} Implementations ===")
    ...
    def _analyze_sequence(self, sequence: List[int], name: str) -> Dict[str, Any]:
        ...
        freq_percentages = {i: round(count / total, 3) for i, count in frequencies.items()}
        ...
        for prev in range(3):
            ...
            if total_from_prev > 0:
                probs = [round(transitions[prev][next_val] / total_from_prev, 3) 
        ...
        # 4. 제약 조건 검증
        violations = sum(1 for i in range(1, len(sequence)) 
                        if sequence[i] == sequence[i-1])
        
        # 5. 균등성 검증 (카이제곱 검정)
        expected = total / 3
        chi_square = sum((count - expected) ** 2 / expected for count in frequencies.values())
    ...
    def _analyze_bias(self, conditional_probs: Dict[int, List[float]]) -> Dict[str, Any]:
        """편향성 분석"""
        bias_results = {}
    ...
    def _generate_comprehensive_report(self, results: Dict[str, Any]):
        ...
        freq_df = pd.DataFrame(freq_data, columns=['Implementation', '0의 빈도', '1의 빈도', '2의 빈도'])
        ...
        cond_df = pd.DataFrame(cond_data, columns=[
            'Implementation', 'Prev 0: [0,1,2]', 'Prev 1: [0,1,2]', 'Prev 2: [0,1,2]', '편향성'
        ])
        ...
        print(f"제약 조건 위반: 총 {sum(violations)}개 (평균 {np.mean(violations):.1f}개/구현)")
    ...
    def _analyze_bias_patterns(self, results: Dict[str, Any]):
        ...
        for impl_id, data in results.items():
            ...
            # 각 구현의 편향 패턴 분류
            uniform_count = sum(1 for info in bias_analysis.values() if '균등' in info.get('type', ''))
            biased_count = sum(1 for info in bias_analysis.values() if '편향' in info.get('type', ''))
    ...
    def export_detailed_report(self, results: Dict[str, Any], filename: str = "detailed_statistical_report.txt"):
        """상세 보고서 텍스트 파일로 출력"""
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 50 + "\n")
            f.write(f"{'코드 번호':<15} {'0의 빈도':<10} {'1의 빈도':<10} {'2의 빈도':<10}\n")
            
            for i, (impl_id, data) in enumerate(results.items(), 1):
                freqs = data['stats']['freq_percentages']
                f.write(f"{i:<15} {freqs[0]:<10.3f} {freqs[1]:<10.3f} {freqs[2]:<10.3f}\n")
            
            # 조건부 확률 표
            f.write(f"\n조건부 확률 (Conditional Probabilities):\n")
            f.write("-" * 80 + "\n")
            f.write(f"{'코드 번호':<10} {'Prev 0: [0,1,2] 확률':<25} {'Prev 1: [0,1,2] 확률':<25} {'Prev 2: [0,1,2] 확률':<25} {'균등 여부':<15}\n")
            ...
            for i, (impl_id, data) in enumerate(results.items(), 1):
                ...
                f.write(f"{i:<10} {str(cond_probs[0]):<25} {str(cond_probs[1]):<25} {str(cond_probs[2]):<25} {bias_summary:<15}\n")
...
def main():
    ...
    if results:
        ...
        print(f"📈 시각화: statistical_analysis.png")
        print(f"📄 상세 보고서: detailed_statistical_report.txt")
    else:
        print("❌ 분석할 구현이 없습니다.")
