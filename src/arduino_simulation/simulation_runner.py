...
class SimulationRunner:
    ...
    def run_parallel_simulation(self, 
        ...
        with ThreadPoolExecutor(max_workers=config.parallel_workers) as executor:
            # 작업 제출
            future_to_seed = {executor.submit(run_single_seed, seed): seed for seed in seeds}
    ...
    def _combine_multiple_results(self, results_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        """다중 시뮬레이션 결과 통합 분석"""
        if not results_list:
            return {}
        
        # 기본 통계
        total_iterations = sum(r['simulation_info']['total_iterations'] for r in results_list)
        total_time = sum(r['simulation_info']['total_time_seconds'] for r in results_list)
        avg_generation_rate = sum(r['simulation_info']['generation_rate_per_second'] for r in results_list) / len(results_list)
        ...
        # 제약 조건 위반 통합
        total_violations = sum(r['constraint_verification']['consecutive_violations'] for r in results_list)
        
        # 전이 분석 통합
        combined_transitions = {}
        for result in results_list:
            for transition, count in result['transition_analysis']['transitions'].items():
                combined_transitions[transition] = combined_transitions.get(transition, 0) + count
        
        # 성능 메트릭 평균
        performance_metrics = {}
        metric_keys = results_list[0]['performance_metrics'].keys()
        for key in metric_keys:
            values = [r['performance_metrics'][key] for r in results_list if key in r['performance_metrics']]
            if values:
        ...
        return {
            ...
            'combined_distribution': {
                ...
                'distribution_variance': self._calculate_distribution_variance(results_list)
            },
            'combined_constraints': {
                'total_violations': total_violations,
                'violation_rate_percent': (total_violations / total_iterations) * 100 if total_iterations > 0 else 0,
    ...
    def _calculate_distribution_variance(self, results_list: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        variance = {}
        for num in [0, 1, 2]:
            percentages = [r['distribution_analysis']['percentages'][num] for r in results_list]
...
def run_multiple_seeds_simulation(seeds: List[int], iterations: int = 10000) -> List[Dict[str, Any]]:
    """다중 시드 시뮬레이션 실행"""
    config = SimulationConfig(
        iterations=iterations,
        show_progress=True,
...
if __name__ == "__main__":
    ...
    print(f"Generation rate: {quick_result['simulation_info']['generation_rate_per_second']:,.0f} gen/sec")
    print(f"Constraint satisfied: {quick_result['constraint_verification']['constraint_satisfied']}")
    
    # 2. 다중 시드 테스트
    print("\n2. Multiple Seeds Test")
    multi_results = run_multiple_seeds_simulation([11111, 22222, 33333], iterations=2000)
