...
class AutoRealArduinoDashboard:
    ...
    def _setup_layout(self):
        ...
        self.app.layout = html.Div([
            # 헤더
            html.Div([
                html.H1("🔥 Real Arduino Implementation Dashboard", 
                       className="text-center mb-4", 
                       style={'color': '#e74c3c', 'textShadow': '2px 2px 4px rgba(0,0,0,0.3)'}),
                html.P("8가지 실제 Arduino C++ 구현 자동 비교 테스트", 
            ...
            # 현재 테스트 중인 구현
            html.Div([
                html.Div(id="current-test-display")
            ], className="mb-4"),
            ...
            # 최종 결과 및 추천
            html.Div([
                html.H3("🏆 Final Results & Recommendation", className="text-center mb-4"),
    ...
    def _setup_callbacks(self):
        ...
        def update_main_display(n_intervals):
            ...
            if self.countdown > 0 and not self.is_running and not self.is_completed:
                ...
                status_text = f"🚀 Real Arduino 테스트가 {self.countdown}초 후 시작됩니다!"
                progress_display = self._create_progress_bar(0, "대기 중...")
                current_test = html.Div()
                return countdown_text, status_text, progress_display, current_test, error_display
            ...
            # 실행 중
                ...
                progress_display = self._create_progress_bar(
                    int((self.progress / len(self.implementations)) * 100) if self.implementations else 0,
            ...
            # 완료됨
                ...
                current_test = html.Div([
                    ...
                    html.P("모든 결과를 아래에서 확인하세요.", className="text-center")
        ...
        @self.app.callback(
            [Output("results-data", "data"),
             Output("realtime-results", "children")],
            [Input("main-interval", "n_intervals")]
        )
        ...
        def update_performance_chart(data):
            ...
            fig = px.bar(df, x='name', y='generation_rate',
        ...
        def update_comparison_chart(data):
            ...
            fig = px.bar(df, x='Implementation', y='Distribution Quality',
        ...
        def update_final_results(data):
            ...
            return html.Div([
                ...
                html.Div([
                    ...
                    html.H3(best['name'], className="text-center text-primary mb-3"),
                    html.H4(f"{best['generation_rate']:,.0f} gen/sec", 
                           className="text-center text-success mb-3"),
                    html.P(f"Type: {best['type']}", className="text-center text-muted")
                ...
                html.Div([
                    ...
                    html.Div([
                        html.H5("⚡ 평균 속도"),
                        html.H3(f"{sum(r['generation_rate'] for r in data) / len(data):,.0f}")
                    ], className="col-md-3 text-center"),
    ...
    def _test_single_implementation(self, impl: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            generation_rate = test_iterations / execution_time if execution_time > 0 else 0
    ...
    def _create_current_test_display(self):
        ...
        return html.Div([
            ...
            html.P(f"진행률: {self.progress}/{len(self.implementations)}", 
    ...
    def _create_realtime_cards(self):
        ...
        for result in self.results[-4:]:  # 최근 4개만 표시
            card = html.Div([
                ...
                html.H4(f"{result['generation_rate']:,.0f}", 
                       className="text-primary"),
                html.P("gen/sec", className="text-muted mb-1"),
                html.Small(f"Violations: {result['violations']}", 
                          className="text-success" if result['violations'] == 0 else "text-danger")
    ...
    def run_server(self):
        """서버 실행"""
        print(f"🔥 Starting Auto Real Arduino Dashboard")
        print(f"📱 URL: http://localhost:{self.port}")
        print(f"⏰ 10초 카운트다운 후 자동 시작")
