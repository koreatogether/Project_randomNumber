...
class ArduinoSimulationDashboard:
    ...
    def _setup_layout(self):
        ...
        self.app.layout = html.Div([
            # 헤더
            html.Div([
                html.H1("Arduino Uno R4 WiFi Random Number Generator Simulation", 
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        html.Button("Stop", id="stop-btn", 
                                  className="btn btn-danger mr-2", disabled=True),
                        html.Button("Load Results", id="load-btn", 
            ...
            # 전이 분석
            html.Div([
                html.H4("Transition Analysis"),
                dcc.Graph(id="transition-chart")
            ], className="mb-4"),
    ...
    def _setup_callbacks(self):
        ...
        def control_simulation(start_clicks, stop_clicks, iterations, seed):
            ...
            if button_id == "start-btn" and start_clicks:
                ...
                return {"running": True, "iterations": iterations, "seed": seed}, True, False
        ...
        def update_status(n_intervals, status_data):
            ...
            if status_data.get("running", False):
                if self.simulation_runner and self.simulation_runner.is_running:
                    ...
                    return html.Div([
                        html.P(f"Running simulation with {status_data['iterations']:,} iterations (seed: {status_data['seed']})"),
                        html.P(f"Progress: {progress.percentage:.1f}% ({progress.current_iteration:,}/{progress.total_iterations:,})"),
                        html.P(f"Rate: {progress.generation_rate:,.0f} gen/sec, Remaining: {progress.estimated_remaining:.1f}s")
                    ])
                else:
        ...
        def update_sequence_chart(data):
            ...
            fig = px.line(df, x='Index', y='Number', 
        ...
        @self.app.callback(
            Output("transition-chart", "figure"),
            [Input("simulation-data", "data")]
        )
        def update_transition_chart(data):
            ...
            fig = px.bar(df, x='Transition', y='Count',
        ...
        def update_performance_table(data):
            ...
            table_data = [
                ...
                ["Generation Rate", f"{sim_info.get('generation_rate_per_second', 0):,.0f} gen/sec"],
                ["Avg Generation Time", f"{perf.get('avg_generation_time_microseconds', 0):.2f} μs"],
                ["Arduino Instructions", f"{perf.get('arduino_instruction_count', 0):,}"],
    ...
    def _create_kpi_cards(self, data: Dict[str, Any]) -> List[html.Div]:
        ...
        cards = [
            ...
            # 제약 조건 만족
            html.Div([
                html.H4("✅" if constraint.get('constraint_satisfied', False) else "❌"),
                html.P("Constraint Satisfied")
            ], className="col-md-3 text-center alert alert-success" if constraint.get('constraint_satisfied', False) else "col-md-3 text-center alert alert-danger"),
...
def create_dashboard(port: int = 8050, debug: bool = True) -> ArduinoSimulationDashboard:
    """대시보드 생성 편의 함수"""
